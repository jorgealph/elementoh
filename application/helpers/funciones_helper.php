...
function Decimal($decin, $numdecimales=3)
{
...
function paginador($obj_query,$pagina=1,$tamaniopag=10,$tipo_boton=1)
{
	/******** Valores devueltos *************/

	$d['registros'] = array();
	//	Indica el número de registros devueltos por la consulta
	$d['total_registros'] = ($obj_query != false) ? $obj_query->num_rows():0;
	//	Indica el número de páginas totales
	...
	{ 
		$d['registros'][$i] = $obj_query->row($i);
	}
	//	Esta dato sirve para el calculo de número de página
	$d['numero_registros'] = $fin - $tamaniopag;
	//	Elemntos HTML (botones de paginación)
	$d['botones_paginacion'] = PintaBotonesPaginacion($d['total_registros'],$pagina,$d['numero_paginas'],5,$tipo_boton);

	return $d;
}

function PintaBotonesPaginacion($num_reg,$num_pag_actual,$num_pags,$num_btn_pag=5,$tipo_boton=1)
{
	$paginacion = '';
	...
	if($tipo_boton == 2)
	{
		...
		{ 
			$paginacion .= '<div class="row">';
			$paginacion .= '<div class="col-md-12 text-center">';
				  $paginacion .= '<div class="site-pagination">';
				    ...
				    for($i=$lim_inicio; $i <= $lim_fin ; $i++)
				    { 
				    	if($num_pag_actual == $i) $paginacion .= '<a href="#" class="active">'.$i.'</a>';
				    	else $paginacion .= '<a href="#" onclick="buscar('.$i.');">'.$i.'</a>';
				    }
