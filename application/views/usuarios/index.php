					...
					<form action="#" id="formbusqueda">
                        <div class="form-body">
                            <div class="row">
                            	...
                            	<div class="col-md-2">
                        			<button class="btn btn-success" type="button" onclick="capturar(0);" style="margin-top:25px;">Crear usuario</button>
                            	</div>
                            </div>
...
<script type="text/javascript">
	function eliminar(id){
        $.post("<?=base_url();?>C_usuarios/eliminar",{id:id},function(resultado,status){
        	if(resultado == "0"){                		
        	 	notificacion('Usuario eliminado','success');
        	 	cargar('<?=base_url()?>C_usuarios/index','#contenido');
        	}
	...
	function buscar(pag,e){
		...
		cargar('<?=base_url();?>C_usuarios/buscar','#contenidomodulo','POST',variables);
	}
    

	function capturar(id){
		var vars = 'id=' + id;
		cargar('<?=base_url();?>C_usuarios/capturar','#contenidomodulo','POST',vars);
	}

    function cambiarPassword(id){
        var vars = 'id=' + id;
        cargar('<?=base_url();?>C_usuarios/cambiar_password','#contenidomodulo','POST',vars);
    }
</script>
</html>